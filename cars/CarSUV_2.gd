...
func _process(delta):
	...
	# f/b input
	speed_input = 0
	speed_input += Input.get_action_strength("accelerate_2")
	speed_input -= Input.get_action_strength("brake_2") 
	speed_input *= acceleration
	# steer input
#	rotate_target = lerp(rotate_target, rotate_input, 5 * delta)
	rotate_input = 0
	rotate_input += Input.get_action_strength("steer_left_2")
	rotate_input -= Input.get_action_strength("steer_right_2")
...
func _on_Lap_area_entered(area):
	queue_free()
